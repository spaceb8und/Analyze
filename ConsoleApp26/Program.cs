...
namespace TestDisplay
{
	class Test
	{
		static void Main(string[] args)
		{
			...
			Utils.Diplay(num); // Output using with Console.WriteLine()
			Utils.Diplay(msg); // Output using with Console.WriteLine()
			Utils.Diplay(c); // Output using with Print()
	...
	class Utils
	{
		...
		public static void Diplay(object item)
		{
			IPrintable ip = item as IPrintable; // the as operator explicitly converts
			// the result of an expression to a given reference or nullable value type
			if (ip != null)
			{
				ip.Print();
			}
